ace.define("ace/mode/asciidoc_highlight_rules", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text_highlight_rules"], function (e, t, n) {
    ...
    r.inherits(o, i), function () { this.foldingStartMarker = /^(?:\|={10,}|[\.\/=\-~^+]{4,}\s*$|={1,5} )/, this.singleLineHeadingRe = /^={1,5}(?=\s+\S)/, this.getFoldWidget = function (e, t, n) { var r = e.getLine(n); return this.foldingStartMarker.test(r) ? r[0] == "=" ? this.singleLineHeadingRe.test(r) ? "start" : e.getLine(n - 1).length != e.getLine(n).length ? "" : "start" : e.bgTokenizer.getState(n) == "dissallowDelimitedBlock" ? "end" : "start" : ""; }, this.getFoldWidgetRange = function (e, t, n) { function l(t) { return f = e.getTokens(t)[0], f && f.type; } function d() { var t = f.value.match(p); if (t)
        return t[0].length; var r = c.indexOf(f.value[0]) + 1; return r == 1 && e.getLine(n - 1).length != e.getLine(n).length ? Infinity : r; } var r = e.getLine(n), i = r.length, o = e.getLength(), u = n, a = n; if (!r.match(this.foldingStartMarker))
        return; var f, c = ["=", "-", "~", "^", "+"], h = "markup.heading", p = this.singleLineHeadingRe; if (l(n) == h) {
        var v = d();
        while (++n < o) {
            if (l(n) != h)
                continue;
    ...
    "use strict";
    var r = e("../lib/oop"), i = e("./text").Mode, s = e("./asciidoc_highlight_rules").AsciidocHighlightRules, o = e("./folding/asciidoc").FoldMode, u = function () { this.HighlightRules = s, this.foldingRules = new o; };
    r.inherits(u, i), function () { this.type = "text", this.getNextLineIndent = function (e, t, n) { if (e == "listblock") {
        var r = /^((?:.+)?)([-+*][ ]+)/.exec(t);
        return r ? (new Array(r[1].length + 1)).join(" ") + r[2] : "";
    } return this.$getIndent(t); }, this.$id = "ace/mode/asciidoc"; }.call(u.prototype), t.Mode = u;
});
