        ...
        'callback' => function ( WP_REST_Request $request ) {
			...
			if( trim( $menuSlug ) !== '' ) {
				...
				if( count( $currentMenu ) > 0 ) {
					$currentMenu = _updateTheReturnedMenuData($currentMenu);
                }
 ...
 * @author Cein
 */
function _updateTheReturnedMenuData( $currentMenu ) {
...
/**
 * Post/page Preview redirect hook for dev, staging and live
 * @author Cein
 */
 ...
 */
add_action( 'rest_api_init', function ($server) {
  $server->register_route( '/', '/metrics', array(
    'methods' => 'GET',
    'callback' => function($request) { return ""; },
  ) );
} );

/**
 * Function to create the output string for the metrics
 * endpoint.
 * @return {str} Metrics output required by prometheus
 * @author John
 */
 ...
 * is adjusted.
 * @param {bool} tells the WP-API that we sent the response
 * @param {str} unused
 * @param {obj} Wordpress request object
 * @param {obj} Wordpress server object
 * @return {bool} 
 * @author John
 */
