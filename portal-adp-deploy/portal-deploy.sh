...
PORTAL_CLOUD_INIT="adp-cloud-config.yml"
...
PORTAL_PRIVATE_NETWORK_ID=$(openstack network list -f value | grep $PRIVATE_NETWORK_NAME | cut -d' ' -f1)
PORTAL_PUBLIC_NETWORK_ID=$(openstack network list -f value | grep $PUBLIC_NETWORK_NAME | cut -d' ' -f1)
